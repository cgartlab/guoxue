...
//   - 69 lessons (num 01..69, all status:'ready') in lessons-manifest.js
...
//   - Mobile (<768px): sidebar hidden, top numstrip visible (flat sorted chips, 69 in total)
...
test.describe('Desktop (1440x900)', () => {
  ...
  test('sidebar renders 5 ready categories (default collapsed)', async ({ page }) => {
    ...
    // Course sub-links: 69 (one per ready lesson)
    await expect(page.locator('.home-sidebar__link--sub')).toHaveCount(69);
  ...
  test('lesson cards render correctly (69 ready lessons)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);

    const cards = page.locator('#lesson-cards .ds-lesson-card');
    await expect(cards).toHaveCount(69);
  ...
  test('series cover stats show correct counts (69/0/5)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Tablet (768x1024)', () => {
  ...
  test('sidebar visible, 69 lesson cards', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Mobile (375x812)', () => {
  ...
  test('sidebar hidden, numstrip visible with 69 chips and horizontal scroll', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
    ...
    // 组内 chip 数：导论=2、学而=16、为政=24、八佾=26、蒙学=1（合计 69）
    const expectedChipCounts = [2, 16, 24, 26, 1];
    ...
    // 全部 69 个序号 chip，按门类顺序排列
    const chips = page.locator('.home-numstrip__chip');
    await expect(chips).toHaveCount(69);
...
test.describe('Interactions', () => {
  ...
  test('click "全部课程" resets to all', async ({ page }) => {
    ...
    await expect(page.locator('#lesson-cards .ds-lesson-card')).toHaveCount(69);
