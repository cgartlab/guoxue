...
//   - 54 lessons (num 01..54, all status:'ready') in lessons-manifest.js
...
//   - Mobile (<768px): sidebar hidden, top numstrip visible (flat sorted chips, 54 in total)
...
test.describe('Desktop (1440x900)', () => {
  ...
  test('sidebar renders 5 ready categories (default collapsed)', async ({ page }) => {
    ...
    // Course sub-links: 54 (one per ready lesson)
    await expect(page.locator('.home-sidebar__link--sub')).toHaveCount(54);
  ...
  test('lesson cards render correctly (54 ready lessons)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
  ...
  test('series cover stats show correct counts (54/0/5)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Tablet (768x1024)', () => {
  ...
  test('sidebar visible, 54 lesson cards', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Mobile (375x812)', () => {
  ...
  test('sidebar hidden, numstrip visible with 54 chips and horizontal scroll', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
    ...
    // 组内 chip 数：导论=2、学而=14、为政=17、八佾=20、蒙学=1（合计 54）
    const expectedChipCounts = [2, 14, 17, 20, 1];
    ...
    // 全部 54 个序号 chip，按门类顺序排列
    const chips = page.locator('.home-numstrip__chip');
    await expect(chips).toHaveCount(54);
...
test.describe('Interactions', () => {
  ...
  test('click "全部课程" resets to all', async ({ page }) => {
    ...
    await expect(page.locator('#lesson-cards .ds-lesson-card')).toHaveCount(54);
