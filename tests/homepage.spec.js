...
//   - 58 lessons (num 01..58, all status:'ready') in lessons-manifest.js
...
//   - Mobile (<768px): sidebar hidden, top numstrip visible (flat sorted chips, 58 in total)
...
test.describe('Desktop (1440x900)', () => {
  ...
  test('sidebar renders 5 ready categories (default collapsed)', async ({ page }) => {
    ...
    // Course sub-links: 58 (one per ready lesson)
    await expect(page.locator('.home-sidebar__link--sub')).toHaveCount(58);
  ...
  test('lesson cards render correctly (58 ready lessons)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
  ...
  test('series cover stats show correct counts (58/0/5)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Tablet (768x1024)', () => {
  ...
  test('sidebar visible, 58 lesson cards', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Mobile (375x812)', () => {
  ...
  test('sidebar hidden, numstrip visible with 58 chips and horizontal scroll', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
    ...
    // 组内 chip 数：导论=2、学而=14、为政=17、八佾=24、蒙学=1（合计 58）
    const expectedChipCounts = [2, 14, 17, 24, 1];
    ...
    // 全部 58 个序号 chip，按门类顺序排列
    const chips = page.locator('.home-numstrip__chip');
    await expect(chips).toHaveCount(58);
...
test.describe('Interactions', () => {
  ...
  test('click "全部课程" resets to all', async ({ page }) => {
    ...
    await expect(page.locator('#lesson-cards .ds-lesson-card')).toHaveCount(58);
