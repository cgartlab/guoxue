...
//   - 59 lessons (num 01..59, all status:'ready') in lessons-manifest.js
...
//   - Mobile (<768px): sidebar hidden, top numstrip visible (flat sorted chips, 59 in total)
...
test.describe('Desktop (1440x900)', () => {
  ...
  test('sidebar renders 5 ready categories (default collapsed)', async ({ page }) => {
    ...
    // Course sub-links: 59 (one per ready lesson)
    await expect(page.locator('.home-sidebar__link--sub')).toHaveCount(59);
  ...
  test('lesson cards render correctly (59 ready lessons)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
  ...
  test('series cover stats show correct counts (59/0/5)', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Tablet (768x1024)', () => {
  ...
  test('sidebar visible, 59 lesson cards', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
...
test.describe('Mobile (375x812)', () => {
  ...
  test('sidebar hidden, numstrip visible with 59 chips and horizontal scroll', async ({ page }) => {
    await page.goto(`http://localhost:${PORT}/`);
    ...
    // 组内 chip 数：导论=2、学而=14、为政=17、八佾=25、蒙学=1（合计 59）
    const expectedChipCounts = [2, 14, 17, 25, 1];
    ...
    // 全部 59 个序号 chip，按门类顺序排列
    const chips = page.locator('.home-numstrip__chip');
    await expect(chips).toHaveCount(59);
...
test.describe('Interactions', () => {
  ...
  test('click "全部课程" resets to all', async ({ page }) => {
    ...
    await expect(page.locator('#lesson-cards .ds-lesson-card')).toHaveCount(59);
